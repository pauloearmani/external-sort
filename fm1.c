...
void selecao_por_substituicaof1(int numRegs, int *comparacoes)
{
    ...
    for (int i = 0; i < quantLerMemoriaInterna; i++)
    {
        ...
        regAux.item = itemAux;
        regAux.marcador = 0;
        inserirf1(heap, regAux, comparacoes); // Adiciona no heap
    }
    ...
    do
    {
        do
        {
            ...
            minReg = extrairMinimof1(heap, comparacoes);
            ...
            inserirf1(heap, novoReg, comparacoes);
...
void initTapes(int numRegs, int *comparacoes)
{
    ...
    selecao_por_substituicaof1(numRegs, comparacoes); // Gera blocos Ordenados
}
...
void binaryToTxt(const char *binaryFileName, const char *txtFileName)
{
    ...
    while (fread(&reg, sizeof(Registerf1), 1, binaryFile) == 1)
    {
...
int intercalateFM1(int *transfRead, int *transfWrite, int *comparacoes)
{
    ...
    while (countTapesFinished < MAX_INPUT_TAPES)
    {
        ...
        for (int i = 0; i < MAX_INPUT_TAPES; i++)
        {
            ...
            short c = fread(&(heap[i].regs), sizeof(Registerf1), 1, inputTapes[i]);
        ...
        fwrite(&(heap[0].regs), sizeof(Registerf1), 1, outputTape);
        *transfWrite = (*transfWrite) + 1;
        ...
        while (countBlocksFinished < MAX_INPUT_TAPES)
        {
            ...
            if (fread(&(heap[0].regs), sizeof(Registerf1), 1, inputTapes[currentTape]) == 0)
            {
...
void formatFinalOutput(FILE *output, FILE *formatedOutput, int show)
{
    ...
    while (fread(&reg, sizeof(Registerf1), 1, output) == 1)
    {
        ...
        {
            fwrite(&reg, sizeof(Registerf1), 1, formatedOutput);
            if (show)
                showReg(reg);
        }
    }
...
void callIntercalate(int *comparacoes)
{
    ...
    while (quantBlocksInOutput > MAX_INPUT_TAPES)
    {
        ...
        Registerf1 regAnterior;
        while (!feof(output))
        {
            fread(&(reg), sizeof(Registerf1), 1, output);
            transferenciasREAD_EXTERNA_PARA_INTERNA = transferenciasREAD_EXTERNA_PARA_INTERNA + 1;
            if (!(regAnterior.mat == -1 && reg.mat == -1))
            {
                fwrite(&(reg), sizeof(Registerf1), 1, inputTapes[index % MAX_INPUT_TAPES]);
...
void switchNameFile(char *nameFile, int situacao)
{
    ...
    {
    case 1:
        strcpy(nameFile, "./arquivos/PROVAOASCENDENTE.TXT");
        break;
    case 2:
        strcpy(nameFile, "./arquivos/PROVAODESCENDENTE.TXT");
        break;
    case 3:
...
int mainf1(int quantidade, int situacao, int opcional)
{
    ...
    /* Formata saida Final*/
    if (opcional == 1) {
        callFormatFinalOutput(opcional);
    }
